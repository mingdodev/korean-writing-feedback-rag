...
def prepare_chroma_data(records):
    ...
    - documents: error_words의 text 필드
    - metadatas: grade, original_sentence, words, error_words의 나머지 필드
    ...
    for record in records:
        ...
        for k, v in record.items():
            if k not in ['error_words', 'file_number']:
                if isinstance(v, (list, dict)):
                    base_metadata[k] = json.dumps(v, ensure_ascii=False)
                else:
                    base_metadata[k] = v
        ...
        # error_words 처리
        error_words = record.get('error_words', [])
        for error_word in error_words:
            # document로 사용할 text 필드 추출
            text = error_word.get('text')
            if not text:
                continue
            ...
            # 기본 메타데이터에 error_word 세부 정보 추가
            metadata = base_metadata.copy()
            metadata.update({
                'error_location': error_word.get('error_location'),
                'error_aspect': error_word.get('error_aspect'),
                'error_level': error_word.get('error_level')
            })
