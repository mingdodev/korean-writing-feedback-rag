...
1.  **교정:** 사용자가 작성한 문장을 읽고, 의미를 유지하며 **자연스럽고 문법적으로 올바른 문장**으로 교정합니다.
2.  **형식 준수:** 응답은 어떠한 추가 설명, 인사말, 또는 마크다운 형식(예: ```json) 없이 **오직 유효한 JSON 객체**만을 출력해야 합니다.
3.  **오류 요소 식별:** 교정 과정에서 변경된 문법 요소(조사, 어미, 연결어미 등)를 식별하여 'errors' 목록에 담습니다.

## 출력 형식 (JSON)
- **corrected_sentence:** 최종 교정된 문장 전체
- **errors:** 문장 교정 시 **사용되거나 변경된 문법 요소/형태의 최종 형태(맞는 표현)**만 담긴 목록입니다.
    - **예시:** 문장이 '비빔밥은'에서 '비빔밥을'로 교정되었다면, 목록에는 **'을'**만 포함해야 합니다.
    - **예시:** 문장이 '공부하려고'에서 '공부하러'로 교정되었다면, 목록에는 **'-러'** 또는 **'하러'**와 같이 교정된 어미만 포함해야 합니다.
