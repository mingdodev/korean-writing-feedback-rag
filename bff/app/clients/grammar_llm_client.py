...
1.  **오류 판단:** 주어진 문장에 문법적, 문맥적 오류가 있는지 판단합니다.
2.  **교정:** 오류가 있다면, 의미를 유지하며 **자연스럽고 문법적으로 올바른 문장**으로 교정합니다.
3.  **형식 준수:** 응답은 어떠한 추가 설명, 인사말, 또는 마크다운 형식(예: ```json) 없이 **오직 유효한 JSON 객체**만을 출력해야 합니다.
4.  **오류 요소 식별:** 교정 과정에서 변경된 문법 요소(조사, 어미, 연결어미 등)를 식별하여 'errors' 목록에 담습니다.

## 출력 형식 (JSON)
- **is_error:** 문장에 교정이 필요한 오류가 있으면 `true`, 없으면 `false`를 설정합니다.
    - **중요:** 문법적으로 완벽하고 자연스러워서 **수정이 전혀 필요 없는 문장**은 `false`로 처리해야 합니다.
- **corrected_sentence:**
    - `is_error`가 `true`이면: 최종 교정된 문장 전체
    - `is_error`가 `false`이면: **원본 문장 그대로**
- **errors:** `is_error`가 `true`일 때, 문장 교정 시 **사용되거나 변경된 문법 요소**만 담긴 목록입니다. `is_error`가 `false`이면 빈 배열 `[]`이어야 합니다.
    - **예시:** 문장이 '비빔밥은'에서 '비빔밥을'로 교정되었다면, 목록에는 **'을'**만 포함해야 합니다.
    - **예시:** 문장이 '공부하려고'에서 '공부하러'로 교정되었다면, 목록에는 **'-러'** 또는 **'-으러'**와 같이 교정된 어미만 포함해야 합니다.
    - **주의:** '틀린 표현 -> 맞은 표현'과 같은 교정 비교 형식은 **절대 포함하지 않습니다.**
    - **예시:** ["을", "에", "-으면", "-으러", "-으세요", "이다"]
"""
